...
const generateToken = (id) =>{
    ...
    let token = jwt.sign(studentId, "Tafiq")
    return token
}
...
const studentSignup = async(req,res)=>{
    ...
    let data = req.body
    let {fName,lName,password,email,sID,mobile,age,gender} = data
    if(!fName || !lName || !password || !email || !sID || !mobile ||!age ||!gender){
...
const studentUpdate = async (req,res ) => {
  ...
  if(result.acknowledged){
    res.status(200).send({ msg: "user data upload"})
  }
  else{
